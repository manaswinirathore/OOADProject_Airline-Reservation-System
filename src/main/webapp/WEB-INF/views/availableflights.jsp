...
<body>
	<div class="container" style="padding: 0;">
		...
		<div class="d-flex flex-column  align-items-center mt-3">
			<c:forEach var="flight" items="${listOfAvailFlights}">
				<div class="card mb-3"  id="flightcard">
					<div class="card-header text-dark">
						<span>${flight.getSource()} - ${flight.getDestination()}</span> <span
							class="float-right">Departure Time:
							${flight.getDepatureTime()}</span>
					</div>
					<div class="card-body text-white">
						<div class="row">
							<div class="col-sm-4 col-4 bg-danger text-center">
								<h6 class="mt-1 text-center card-title">${flight.getFlightName()}</h6>
								<h6>${flight.getFlightNo()}</h6>
							</div>
							...
							<div
								...
								<c:choose>
									<c:when
										test="${flight.getEconomy() eq 0 and (flight.getBusiness() eq 0) and (flight.getFirstClass() eq 0
										)}">
										<a class="btn text-white" href="#">Not Available</a>
									</c:when>

									<c:otherwise>
										<a class="btn text-white" href="/book/${flight.getFlightNo()}/">Book
											Now</a>
									</c:otherwise>
								</c:choose>
